...
faqs:
  - question: "Why can't we just use Shadcn or another component library?"
    answer: "<strong>You can, and should if it fits.</strong> But here's what we see: Teams use Shadcn, then ask v0 to generate features. v0 generates code that doesn't match Shadcn patterns. Now you're retrofitting AI output to fit Shadcn. Our approach works differently. We create guidelines and prompts that generate code matching your needs. The AI becomes the implementation layer, not something you're fighting against. If Shadcn works for you, great. We often build systems that complement Shadcn rather than replace it."
  - question: "Isn't AI going to make design systems obsolete?"
    answer: "<strong>The opposite.</strong> AI makes design systems more important, not less. AI can generate any design you describe. That's the problem. Without constraints, every generation is different. Design systems are the constraints that make AI output consistent. They're the vocabulary AI uses to understand what \"on-brand\" means for your product. Teams without design systems produce inconsistent products faster. Teams with AI-friendly design systems produce consistent products faster. Speed and consistency together."
  - question: "What if we hire a designer later?"
    answer: "<strong>The system helps you hire better.</strong> You're not asking designers to fix a mess. You're asking them to evolve an existing system. That's more attractive to senior designers. The system we build is designed to be extended, not replaced. When you hire a designer, they'll have a foundation to build on rather than starting from scratch."
  - question: "Do you maintain the system as our product evolves?"
    answer: "<strong>First 30 days:</strong> Minor adjustments included (tweaking colors, refining patterns). <strong>After that:</strong> Three options: 1. Maintain it yourself (it's built to be maintainable by engineers) 2. Monthly subscription for ongoing updates ($5-8K/month) 3. Project-based updates when you need major additions. Most teams maintain simple patterns themselves and hire us for major expansions (adding mobile guidelines, new product surfaces, etc.)."
  - question: "What if AI tools change their APIs or capabilities?"
    answer: "<strong>We document principles, not tool-specific tricks.</strong> The core of the system (colors, spacing, patterns) doesn't change when Cursor updates. The prompts might need tweaking, but the foundation stays stable. We also version the prompt library. When tools evolve, we can update prompts without rebuilding the entire system."
  - question: "Can this work with our existing tech stack?"
    answer: "<strong>Yes. We're framework-agnostic.</strong> React, Vue, Next.js, Svelte doesn't matter. The principles work across all of them. We typically deliver systems as Tailwind design tokens (most flexible) but can adapt to CSS variables, styled-components, or whatever your stack uses."
---

<h2 className="flex items-start gap-3"> <span className="flex items-center gap-3"><span className="icon-container rose"><ExclamationTriangleIcon/></span></span> Here's what we're seeing from AI-assisted dev teams</h2>
...
<h2 className="flex items-center gap-3"> <span className="flex items-center gap-3"><span className="icon-container yellow"><LightBulbIcon/></span></span> Questions for AI-assisted teams</h2>
